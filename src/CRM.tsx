...
export default function CRM() {
  ...
  // ===== Funciones dinámicas de equipos =====
  const getAvailableManagers = () => {
    return users.filter((u: any) => u.role === "gerente" && u.active !== false);
  };

  const getBotConfig = () => {
    const managers = getAvailableManagers();
    const config: Record<string, { targetTeam: string | null; label: string }> = {};
    
    // Crear configuración dinámica para bots por gerente
    managers.forEach((manager, index) => {
      const botKey = `whatsapp_bot_${manager.name.toLowerCase().replace(/\s+/g, '_')}`;
      config[botKey] = {
        targetTeam: manager.name.toLowerCase(),
        label: `Bot ${manager.name}`
      };
    });
    
    // Bot 100 siempre va a distribución general
    config['whatsapp_100'] = { targetTeam: null, label: "Bot 100" };
  ...
  // ===== Función para obtener usuarios visibles según rol =====
  ...
  const getVendorsByTeamName = (teamName: string) => {
  ...
  const pickNextVendorId = (scopeUser?: any, botSource?: string) => {
    ...
    if (botSource && botConfig[botSource]) {
      ...
      if (botConf.targetTeam) {
        pool = getVendorsByTeamName(botConf.targetTeam);
      } else {
  ...
  const downloadAllLeadsExcel = () => {
    ...
    const excelData = filteredLeads.map(lead => {
      ...
      const fuente = fuentes[lead.fuente as string] || { label: lead.fuente };
      ...
      return {
        ...
        'Última Actualización': lead.historial && lead.historial.length > 0 
          ? formatDate(lead.historial[lead.historial.length - 1].timestamp)
          : ''
      };
    });
    ...
    const colWidths = [
      { wch: 8 }, { wch: 20 }, { wch: 15 }, { wch: 20 }, { wch: 15 }, { wch: 20 }, 
      { wch: 10 }, { wch: 15 }, { wch: 15 }, { wch: 20 }, { wch: 20 }, { wch: 12 }, 
      { wch: 30 }, { wch: 18 }
    ];
    ws['!cols'] = colWidths;
    ...
    const teamName = teamFilter && teamFilter !== "todos" 
      ? `_${getTeamManagerById(teamFilter)?.name || 'equipo'}`
      : '';
    const filename = `leads_completo${teamName}_${today}.xlsx`;

    XLSX.writeFile(wb, filename);
  };

  const downloadLeadsByStateExcel = (estado: string) => {
    if (!currentUser) return;
    ...
    const colWidths = [
      { wch: 8 }, { wch: 20 }, { wch: 15 }, { wch: 20 }, { wch: 15 }, { wch: 20 }, 
      { wch: 10 }, { wch: 15 }, { wch: 20 }, { wch: 20 }, { wch: 12 }, { wch: 30 }, { wch: 50 }
    ];
    ws['!cols'] = colWidths;

    const sheetName = `Leads ${estados[estado]?.label || estado}`;
    ...
    const teamName = teamFilter && teamFilter !== "todos" 
      ? `_${getTeamManagerById(teamFilter)?.name || 'equipo'}`
      : '';
    const filename = `leads_${estado}${teamName}_${today}.xlsx`;

    XLSX.writeFile(wb, filename);
  };

  const downloadStatesSummaryExcel = () => {
    if (!currentUser) return;
    ...
    const summaryData = Object.entries(estados).map(([key, estado]) => {
      const leadsInState = filteredLeads.filter(l => l.estado === key);
      const percentage = filteredLeads.length > 0 
        ? ((leadsInState.length / filteredLeads.length) * 100).toFixed(1)
        : '0';
      
      return {
        'Estado': estado.label,
        'Cantidad': leadsInState.length,
        'Porcentaje': `${percentage}%`
      };
    });

    const summarySheet = XLSX.utils.json_to_sheet(summaryData);
    summarySheet['!cols'] = [{ wch: 20 }, { wch: 12 }, { wch: 12 }];
    XLSX.utils.book_append_sheet(wb, summarySheet, "Resumen por Estados");

    Object.entries(estados).forEach(([key, estado]) => {
      const leadsInState = filteredLeads.filter(l => l.estado === key);
      
      if (leadsInState.length > 0) {
        const stateData = leadsInState.map(lead => {
          const vendedor = lead.vendedor ? userById.get(lead.vendedor) : null;
          const fuente = fuentes[lead.fuente as string] || { label: lead.fuente };
        ...
        XLSX.utils.book_append_sheet(wb, stateSheet, estado.label);
      }
    });
    ...
    const teamName = teamFilter && teamFilter !== "todos" 
      ? `_${getTeamManagerById(teamFilter)?.name || 'equipo'}`
      : '';
    const filename = `resumen_estados${teamName}_${today}.xlsx`;
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    {/* Opciones dinámicas para bots por gerente */}
                    {getAvailableManagers().map((manager, index) => (
                      <option 
                        key={`bot_${manager.id}`} 
                        value={`whatsapp_bot_${manager.name.toLowerCase().replace(/\s+/g, '_')}`}
                      >
                        💬 Bot {manager.name} (Equipo {manager.name})
                      </option>
                    ))}
                  ...
                  <div className="mt-2 text-xs space-y-1">
                    {getAvailableManagers().map((manager) => (
                      <div key={manager.id} className="text-green-600">
                        💬 Bot {manager.name} → Equipo {manager.name} automáticamente
                      </div>
                    ))}
      ...
      <div className="flex-1 p-6">
            ...
            <div className="flex items-center justify-between">
              ...
              {["owner", "director"].includes(currentUser?.role) && (
                ...
                >
                  ...
                  {getAvailableManagers().map((gerente: any) => (
        ...
        {activeSection === "leads" && (
          <div className="space-y-6">
                ...
                {["owner", "director"].includes(currentUser?.role) && (
                  <div className="relative group">
                    <button
                      ...
                      title="Descargar Excel con todos los leads visibles"
                    >
                    ...
                    <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-800 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
                      Descargar todos los leads en formato Excel
            ...
            {["owner", "director"].includes(currentUser?.role) && getFilteredLeads().length > 0 && (
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-medium text-gray-800">Exportar Datos</h4>
                    <p className="text-sm text-gray-600">
                      {getFilteredLeads().length} leads disponibles para descarga
                    </p>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      onClick={downloadStatesSummaryExcel}
                  ...
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <button
                        ...
                        title="Descargar Excel con todos los leads del equipo seleccionado"
                      >
                      ...
                      <button
                        ...
                        title="Descargar Excel con resumen por estados y hojas separadas por cada estado"
                      >
                        <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2-2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                        </svg>
                      ...
                      {selectedEstado && (
                        <button
                          onClick={() => downloadLeadsByStateExcel(selectedEstado)}
                          className={`flex items-center justify-center px-4 py-2 text-white rounded-lg transition-colors ${estados[selectedEstado].color} hover:opacity-90`}
                          title={`Descargar Excel solo con leads en estado: ${estados[selectedEstado].label}`}
                        >
                          <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M9 19l3 3m0 0l3-3m-3 3V10" />
                          </svg>
                          Solo {estados[selectedEstado].label}
                        </button>
                      )}
                    </div>
                    ...
                    <div className="mt-3 text-xs text-green-700">
                      <p className="mb-1">
                        <span className="font-medium">Todos los Leads:</span> Descarga completa con todas las columnas y datos
                      </p>
                      <p className="mb-1">
                        <span className="font-medium">Resumen Estados:</span> Excel con múltiples hojas - resumen general + hoja por cada estado
                      </p>
                      {selectedEstado && (
                        <p>
                          <span className="font-medium">Solo {estados[selectedEstado].label}:</span> Únicamente leads en este estado con historial detallado
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  {Object.entries(estados).map(([key, estado]) => {
                    ...
                    return (
                      <div key={key} className="relative group">
                        ...
                        >
                          <div className={`${estado.color} text-white rounded-lg p-4 mb-2 hover:opacity-90 relative`}>
