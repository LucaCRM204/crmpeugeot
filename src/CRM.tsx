              ...
              <tr key={lead.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <div>
                    ...
                    {lead.entrega && (
                      <div className="text-xs text-green-600">✓ Entrega usado</div>
                    )}
                  </div>
                </td>
                <td className="px-6 py-4">
                    ...
                    <div>
                      <div
                        className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium text-white ${
                          fuentes[lead.fuente as string]?.color || "bg-gray-400"
                        }`}
                      >
                        {fuentes[lead.fuente as string]?.label || String(lead.fuente)}
                      </div>
                        ...
                        try {
                          ...
                          if (mapped.vendedor)
                            pushAlert(
                              mapped.vendedor,
                              "lead_assigned",
                              `Te reasignaron el lead: ${mapped.nombre}`
                            );
                        } catch (err) {
                              ...
                              <div key={index} className="text-xs flex items-center space-x-1">
                                <span
                                  className={`px-1 py-0.5 rounded text-white text-[10px] ${
                                    estados[entry.estado]?.color || "bg-gray-400"
                                  }`}
                                >
            ...
            return (
              <tr key={user.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-sm text-gray-900">
                  {user.reportsTo ? userById.get(user.reportsTo)?.name : "—"}
                </td>
          ...
          <div>
            ...
            <span
              className={`inline-block px-2 py-1 rounded-full text-xs font-medium text-white ${
                estados[viewingLeadHistorial.estado].color
              }`}
            >
