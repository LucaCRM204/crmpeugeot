...
export default function CRM() {
  ...
  const getAccessibleUserIds = (user: any) => {
    ...
    if (["owner", "gerente_general"].includes(user.role))
  ...
  const getVisibleUsers = () => {
    ...
    return users.filter((u: any) => {
      ...
      // gerente_general ve a todos menos al owner
                    ...
                    <div key={key} className="relative group">
                      ...
                      >
                        <div className={`${estado.color} text-white rounded-lg p-4 mb-2 relative cursor-pointer hover:opacity-90 transition-opacity`}>
                          ...
                          {/* Botón de descarga solo para owner y gerente_general */}
                          {["owner", "gerente_general"].includes(currentUser?.role) && count > 0 && (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                const teamFilter = ["owner", "gerente_general"].includes(currentUser?.role)
