    ...
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <div className="flex-1 p-6">
        ...
        {activeSection === "seguimiento" && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-3xl font-bold text-gray-800">Seguimiento de Cambios</h2>
              <div className="flex items-center space-x-2 text-sm text-gray-600">
                <BarChart3 size={16} />
                <span>{seguimiento.length} registros</span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg">
              <div className="px-6 py-4 border-b border-gray-200">
                <p className="text-sm text-gray-600">
                  Registro automático de todos los cambios de estado de leads. 
                  {currentUser?.role !== "owner" && (
                    <span className="text-orange-600 ml-1">Solo el dueño puede eliminar registros.</span>
                  )}
                </p>
              </div>
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Cambio de Estado</th>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {seguimiento.map((entrada) => {
                        const lead = leads.find(l => l.id === entrada.leadId);
                        const fechaFormatted = new Date(entrada.timestamp).toLocaleString("es-AR", {
                          day: "2-digit",
                          month: "2-digit", 
                          year: "numeric",
                          hour: "2-digit",
                        ...
                        return (
                          <tr key={entrada.id} className="hover:bg-gray-50">
                            <td className="px-6 py-4 text-sm text-gray-900">
                              <div className="font-medium">{fechaFormatted}</div>
                            </td>
                            <td className="px-6 py-4 text-sm">
                              <div className="flex items-center">
                                <div>
                                  <div className="font-medium text-gray-900">{entrada.usuario}</div>
                                  <div className="text-xs text-gray-500">
                                    {roles[users.find(u => u.id === entrada.usuarioId)?.role] || "Usuario"}
                                  </div>
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="px-6 py-4 text-sm">
                              <div>
                                <div className="font-medium text-gray-900">{lead?.nombre || "Lead eliminado"}</div>
                                {lead && (
                                  <div className="text-xs text-gray-500">
                                    {lead.telefono} • {lead.modelo}
                                  </div>
                                )}
                              </div>
                            </td>
                            <td className="px-6 py-4 text-sm">
                              {entrada.estadoAnterior && entrada.estadoNuevo && (
                                <div className="flex items-center space-x-2">
                                  <span className={`px-2 py-1 rounded-full text-xs font-medium text-white ${estados[entrada.estadoAnterior]?.color || 'bg-gray-500'}`}>
                                    {estados[entrada.estadoAnterior]?.label || entrada.estadoAnterior}
                                  </span>
                                  <span className="text-gray-400">→</span>
                                  <span className={`px-2 py-1 rounded-full text-xs font-medium text-white ${estados[entrada.estadoNuevo]?.color || 'bg-gray-500'}`}>
                                    {estados[entrada.estadoNuevo]?.label || entrada.estadoNuevo}
                                  </span>
                                </div>
                              )}
                            </td>
                            <td className="px-6 py-4 text-sm text-gray-600">
                              <div className="max-w-xs truncate" title={entrada.notas}>
                                {entrada.notas}
                              </div>
                            </td>
                            {currentUser?.role === "owner" && (
                              <td className="px-6 py-4 text-right text-sm">
                                <button
                                  onClick={() => setSeguimiento(prev => prev.filter(s => s.id !== entrada.id))}
                                  className="inline-flex items-center px-2 py-1 text-xs rounded-md bg-red-100 text-red-700 hover:bg-red-200"
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-white rounded-lg shadow p-4">
                ...
                <p className="text-2xl font-bold text-blue-600">{seguimiento.length}</p>
              </div>
              <div className="bg-white rounded-lg shadow p-4">
